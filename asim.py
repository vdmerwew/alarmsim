...
class SensorSimulationApp:
    def __init__(self, root):
        ...
        self.dragging_sensor = False  # Track if a sensor is being dragged
        self.intruder = None  
        self.dragging_intruder = False  # Track if an intruder is being dragged
        self.drone = None  # Track the drone
        self.dragging_drone = False  # Track if the drone is being dragged
        self.panel = None  # Initialize panel
        self.dragging_panel = False  # Track if the panel is being dragged
        self.gps_points = {}
    ...
    def on_drone_press(self, event):
        print("Drone clicked!")  # Debugging message
        self.dragging_drone = True  # Set flag to indicate that the drone is being dragged
        # Store the initial position of the mouse
        self.drone_last_x = event.x
        self.drone_last_y = event.y

    def on_drone_move(self, event):
        if self.dragging_drone:  # Only allow dragging if the flag is set
            dx = event.x - self.drone_last_x
            dy = event.y - self.drone_last_y

            # Move the drone by the delta values
            self.canvas.move(self.drone_id, dx, dy)
    ...
    def on_drone_release(self, event):
        if self.dragging_drone:  # Only update location if the drone was being dragged
            # Get the current coordinates of the drone
            x, y = self.canvas.coords(self.drone_id)[:2]  # Get new position
            self.drone.current_location = (x + 10, y + 10)  # Update drone's location
            print(f"Drone released at: {self.drone.current_location}")
        self.dragging_drone = False  # Reset dragging flag
    ...
    def delete_panel(self):
        if self.panel:
            self.panel = None  # Remove reference to the panel
    ...
    def on_canvas_click(self, event):
        ...
        if self.drone:
            ...
            if drone_x <= event.x <= drone_x + 30 and drone_y <= event.y <= drone_y + 30:
                print("Drone clicked!")
                self.dragging_drone = self.drone  # Start dragging the drone
                return
    ...
    def on_mouse_drag(self, event):
        ...
        # Check if dragging a drone
            ...
            current_x, current_y = self.dragging_drone.current_location  
            dx = event.x - self.prev_x
            dy = event.y - self.prev_y
            new_x = current_x + dx
            new_y = current_y + dy
            self.dragging_drone.current_location = (new_x, new_y)  # Update the current location tuple
        ...
        # Always update prev_x and prev_y after handling the event
        self.prev_x = event.x
        self.prev_y = event.y
    ...
    def on_mouse_release(self, event):
        # Only reset dragging_panel if necessary
        if self.dragging_panel:
            self.dragging_panel = None  # Reset panel dragging
        self.dragging_sensor = None
        self.dragging_intruder = None  
        self.dragging_drone = None  
        self.dragging = False
    ...
    def detect_sensors(self):
        ...
        for sensor in detected_sensors:
            ...
            gps_button = tk.Button(sensors_window, text="Add GPS Points", command=lambda s=sensor: self.open_add_gps_points(s))
            gps_button.pack()

        # Enable "Send GPS to Drone" after sensors are detected
        self.send_gps_btn.config(state=tk.NORMAL)

    def open_add_gps_points(self, sensor):
        # Create a new window for adding GPS points
        gps_window = Toplevel(self.root)
        gps_window.title(f"Add GPS Points for {sensor.name}")

        # Create a canvas in the new window
        gps_canvas = tk.Canvas(gps_window, width=800, height=600, bg="white")
        gps_canvas.pack()

        # Draw the existing sensors and their FOV on this new canvas
        self.draw_existing_sensors(gps_canvas)

        # List of GPS points for this sensor
        self.gps_points[sensor.name] = []

        # When the user clicks on the canvas, record the GPS point
        gps_canvas.bind("<Button-1>", lambda event: self.add_gps_point(event, sensor, gps_canvas))

        # Add save button to finalize GPS points
        tk.Button(gps_window, text="Save", command=lambda: self.save_gps_points(sensor)).pack()


    def draw_existing_sensors(self, canvas):
        for sensor in self.sensors:
            # Draw each sensor as a rectangle on the canvas
            canvas.create_rectangle(sensor.x, sensor.y, sensor.x + 25, sensor.y + 25, fill="blue")
            
            # Draw the FOV of the sensor (assuming circular FOV)
            fov_radius = sensor.detection_range  # Assume this is defined in your sensor
            canvas.create_oval(sensor.x - fov_radius, sensor.y - fov_radius,
                            sensor.x + 25 + fov_radius, sensor.y + 25 + fov_radius,
                            outline="green", width=2)  # Draw FOV as a circle
    
    def is_in_sensor_fov(self, sensor, point):
        # Check if the point is within the sensor's FOV
        sensor_center = (sensor.x + 12.5, sensor.y + 12.5)  # Center of the sensor
        radius = sensor.detection_range  # Assume this is defined in your sensor
        distance = ((sensor_center[0] - point[0]) ** 2 + (sensor_center[1] - point[1]) ** 2) ** 0.5
        return distance <= radius
    
    def draw_gps_point(self, gps_canvas, x, y):
        # Draw a small circle to represent the GPS point
        gps_canvas.create_oval(x - 5, y - 5, x + 5, y + 5, fill="red")
      
   
    
    def add_gps_points(self, sensor):
        # Ensure the sensor has a defined FOV
        if not sensor.is_enabled:
            messagebox.showinfo("Sensor Disabled", "Cannot add GPS points to a disabled sensor.")
            return

        # List of GPS points for this sensor
        self.gps_points[sensor.name] = []

        # Inform the user that they can click on the canvas to add GPS points
        print(f"Click on the canvas to add GPS points for {sensor.name}.")
        
        # Define a callback to handle GPS point addition on the main canvas
        def on_click(event):
            # Get the coordinates where the user clicked
            x, y = event.x, event.y

            # Optionally check if the click is within the sensor's FOV
            if self.is_within_fov(sensor, x, y):
                ...
                print(f"Click at ({x}, {y}) is outside the FOV of {sensor.name}.")

        # Bind the click event to the main canvas
        self.canvas.bind("<Button-1>", on_click)

        # Add a button to finalize GPS points (optional)
        tk.Button(self.root, text="Done Adding GPS Points", command=lambda: self.finish_adding_gps_points(sensor)).pack()

    def finish_adding_gps_points(self, sensor):
        # Unbind the click event to stop adding GPS points
        self.canvas.unbind("<Button-1>")
        print(f"Finished adding GPS points for {sensor.name}.")
        # Optionally save or process the GPS points
        self.save_gps_points(sensor)
