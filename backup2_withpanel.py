...
class SensorSimulationApp:
    ...
    def detect_sensors(self):
        # Simulate detecting sensors (in real life, would be via WiFi)
        detected_sensors = [sensor.name for sensor in self.sensors]

        # Create a new window to list detected sensors and assign GPS points
        sensors_window = Toplevel(self.root)
        sensors_window.title("Detected Sensors")

        for sensor in self.sensors:
            sensor_label = Label(sensors_window, text=sensor.name)
            sensor_label.pack()

            # Add a button next to each sensor to add GPS points
            gps_button = Button(sensors_window, text="Add GPS Points", command=lambda s=sensor: self.add_gps_points(s))
            gps_button.pack()

        # Enable "Send GPS to Drone" button after sensors are detected
